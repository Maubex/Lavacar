    ...
    div.innerHTML = `
        <div class="resultado-box nao-encontrado">
            ...
            <div class="row mt-3">
                ...
                <div class="col-md-6">
                    <a href="/clientes/?novo=true" class="btn-secundario w-100 text-center d-block">
                        + Cadastrar cliente
                    </a>
                </div>
            </div>
...
function exibir_card_carro(carro) {
    ...
    let historico_html = ''
    if (carro.historico.length === 0) {
        historico_html = `<tr><td colspan="4" class="text-center text-muted">Nenhum serviço registrado ainda.</td></tr>`
    } else {
        carro.historico.forEach(h => {
            const badge = h.tipo === 'Lavagem'
                ? `<span class="badge-lavagem">${h.tipo}</span>`
                : `<span class="badge-estetica">${h.tipo}</span>`
            historico_html += `
                <tr>
                    <td>${badge}</td>
                    <td>${h.descricao}</td>
            ...
            <div class="carro-info">
                <div class="row">
                    ...
                    <div class="col-md-4 text-right">
                        ...
                        <!-- ✅ NOVO — Botão principal de OS -->
                        <a href="/servicos/carro/${carro.id}/nova-os/" class="btn-servico btn-nova-os">
                            🧾 Nova OS
                        </a>

                        <!-- Botões legados -->
                        <a href="/servicos/carro/${carro.id}/lavagem/" class="btn-servico btn-lavagem">
                            🚿 Lavagem
                        </a>
                        <a href="/servicos/carro/${carro.id}/estetica/" class="btn-servico btn-estetica">
                            ✨ Estética
                        </a>
...
// ── Busca automática se vier ?placa= na URL ──────
document.addEventListener('DOMContentLoaded', () => {
    const params = new URLSearchParams(window.location.search)
    const placa  = params.get('placa')
    if (placa) {
        document.getElementById('busca-input').value = placa
        buscar_atendimento()
    }
})